...
public class LottoV2
{
	public static void main(String[] args)
	{
		...
		System.out.println("Welcome to the Java-lottery (version 2)!");
		...
		if(x.length != 0) //Verifies that x is not empty. If empty, the else-statement will print out an error
		{
			...
			{
			draw(x,maxDigit);
			bubbleSort(x); //Sorts the table before print
			print(x); //Prints!
			}
		}
		else
	...
	static void bubbleSort (int[] table) //Bubble sort function, checks for biggest number of an integer i and the integer above in array
	{
		int pass, i, max;
